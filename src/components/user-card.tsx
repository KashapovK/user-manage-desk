  ...
  return (
    <Card className="p-6 flex flex-col gap-4 bg-card border-border hover:shadow-lg transition-shadow">
      <div className="flex items-start gap-4">
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-primary-foreground text-xl font-semibold shrink-0">
      ...
      <div className="flex gap-2 pt-2 border-t border-border">
        <Button variant="outline" size="sm" onClick={handleEdit} className="flex-1 gap-2 bg-transparent">
          <Edit className="w-4 h-4" />
          Редактировать
        </Button>
        {user.status === "active" ? (
          <>
            <Button variant="outline" size="sm" onClick={handleArchive} className="gap-2 bg-transparent">
              <Archive className="w-4 h-4" />
              Архив
            </Button>
            <Button variant="outline" size="sm" onClick={handleHide} className="gap-2 bg-transparent">
              <EyeOff className="w-4 h-4" />
              Скрыть
            </Button>
          </>
        ) : user.status === "archived" ? (
          <Button variant="outline" size="sm" onClick={handleUnarchive} className="gap-2 bg-transparent">
